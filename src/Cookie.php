 ...
 * Cookie 操作封装。
 *
 * -- 使用 Cookie 时请注意个数不要超过浏览器限制。
 * -- 使用 Cookie 时所有 Cookie 数值累加不要超过浏览器限制。
 *
 * @author fingerQin
 * @date 2019-12-06
 */
...
class Cookie
{
    ...
    {
        $value = Crypt::sys_auth($value, 'ENCODE');
    ...
    public static function get($name, $value = '')
    {
        if (isset($_COOKIE[$name])) {
            $cookie = $_COOKIE[$name];
            return Crypt::sys_auth($cookie, 'DECODE');
        } else {
            return $value;
        }
    }
    ...
    public static function all()
    {
        ...
        foreach ($cookies as $name => $cookie) {
            $cookies[$name] = Crypt::sys_auth($cookie, 'DECODE');
        }
        return $cookies;
    }
